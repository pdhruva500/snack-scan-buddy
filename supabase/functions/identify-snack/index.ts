// @ts-ignore: using Deno std remote import in Supabase Edge Function
import { serve } from "https://deno.land/std@0.168.0/http/server.ts";

// Declare Deno for local type-checking environments
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const data = await response.json();
    const snackName = data?.choices?.[0]?.message?.content?.trim() || 'Unknown Snack';
